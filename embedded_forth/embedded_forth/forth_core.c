...
int fvm_mode;
fvm_put_char_handler fvm_put_char_proc;
...
void fvm_init(fvm_data *fvm, fvm_put_char_handler put_char_func, const forth_word *_last_word)
{
  fvm_last_word_initial = _last_word;
  fvm_put_char_proc = put_char_func;
  fvm_reinit(fvm);
}
