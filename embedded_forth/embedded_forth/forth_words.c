...
int fvm_get_string(void (*handler)(char))
{
  char c;
    ...
    while ((c = *fvm_command_buffer_current++) != 0)
    {
      switch (c)
      {
        case '\\':
          ...
          switch (c)
          {
            case 'r': handler('\r'); break;
            case 'n': handler('\n'); break;
            case 't': handler('\t'); break;
            case '\\': handler('\\'); break;
            default:
...
void string_save_handler(char c)
{
  *char_p++ = c;
}
...
int point_quote_word_handler2(fvm_data *fvm)
{
  ...
  {
    if (!fvm_get_string(fvm_put_char_proc))
      return FVM_ERROR;
    return FVM_FALSE;
  }
